...
- {{读者人群}}：${options.audience}
  - 角色/画像（示例：泛科技读者、产品经理、券商分析师、资深工程师、小学/初中/高中老师）
  - 背景知识水平：入门 / 一般
...
## 搜索引擎选择（运行时传入）
- {{搜索引擎}}：${options.engine}（默认 Tavily）
- 密钥来源（仅用于调用，严禁在任何输出中展示或回显）：
  - Tavily：从环境变量 \`TAVILY_API_KEY\` 读取
  - Exa：从环境变量 \`EXA_API_KEY\` 读取
- 调用策略：
  - 按 {{搜索引擎}} 指定的提供者发起检索；请求超时（≥8s）、429/5xx、或有效结果数量 < 3 时，
    自动切换到另一提供者 **只重试一次**；仍失败则返回"检索失败：{provider}，原因：{错误码/超时}"，并停止写作。
- 统一字段规范（内部使用，不在正文展示）：
  - 将不同提供者的结果标准化为：\`title, url, snippet, published_at, region, provider\`
  - 去重策略：同域名仅保留相关性最高的 1–2 条；相似标题 Jaccard ≥0.8 的只保留一条。
- 合规与隐私：
  - 禁止在聊天输出、Front‑matter、或中间分析中打印任何 API Key。
  - 若用户在输入中包含密钥，**也不回显**；仅用于请求。
